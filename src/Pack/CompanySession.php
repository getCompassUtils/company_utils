...
class CompanySession {
	...
	public const MAP_ENTITY_TYPE = "company_session";
	...
	public static function doEncrypt(string $session_map):string {
		...
		if (isset($GLOBALS[__CLASS__]["key_list"][$session_map])) {
		...
		$iv          = substr(CryptProvider::session()->vector(), 0, $iv_length);
		$session_key = openssl_encrypt($json, ENCRYPT_CIPHER_METHOD, CryptProvider::session()->key(), 0, $iv);

		$GLOBALS[__CLASS__]["key_list"][$session_map] = $session_key;
		return $GLOBALS[__CLASS__]["key_list"][$session_map];
	}
	...
	public static function doDecrypt(string $session_key):string {
		...
		if (isset($GLOBALS[__CLASS__]["map_list"][$session_key])) {
			return $GLOBALS[__CLASS__]["map_list"][$session_key];
		}

		// расшифровываем
		$iv_length      = openssl_cipher_iv_length(ENCRYPT_CIPHER_METHOD);
		$iv             = substr(CryptProvider::session()->vector(), 0, $iv_length);
		$decrypt_result = openssl_decrypt($session_key, ENCRYPT_CIPHER_METHOD, CryptProvider::session()->key(), 0, $iv);
		...
		$GLOBALS[__CLASS__]["map_list"][$session_key] = $decrypt_result["company_session_map"];
	...
	protected static function _convertMapToPacket(string $session_map):array {
		...
		if (isset($GLOBALS[__CLASS__]["packet_list"][$session_map])) {
			return $GLOBALS[__CLASS__]["packet_list"][$session_map];
		}
		...
		$GLOBALS[__CLASS__]["packet_list"][$session_map] = $output;
		return $output;
	}
	...
	protected static function _getSignature(array $packet):string {
		...
		$sign     = hash_hmac("sha1", $string_for_sign, PackCryptProvider::company()->salt($version));
